        ...
        static void Main(string[] args)
        {
            using (WebClient myWebClient = new WebClient())
            {
                ...
                if (bool.Parse(ConfigurationManager.AppSettings["UseVLC"]))
                {
                    ...
                    Process.Start(ConfigurationManager.AppSettings["VLCPath"], $"--fullscreen --qt-fullscreen-screennumber=2 http://127.0.0.1:{ConfigurationManager.AppSettings["AcePort"]}/ace/getstream?id={matches[0].Groups[1].Value}&preferred_audio_language=rus ");
                }
                else
                {
